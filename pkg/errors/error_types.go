...
// ErrorCode is a type alias for errorcode.ErrorCode
...
func (e *NessiError) Error() string {
	baseMsg := fmt.Sprintf("[%s] %s: %s", e.Code, errorcode.GetErrorDescription(e.Code), e.Message)
...
func NewError(code ErrorCode, message string, details ...string) *NessiError {
	...
	if len(details) > 0 {
		nessiErr.Details = details[0]
	}

	return nessiErr
}

// WrapError creates a new NessiError that wraps an existing error
func WrapError(err error, code ErrorCode, message string, details ...string) *NessiError {
	nessiErr := NewError(code, message, details...)
	return nessiErr.WithWrappedError(err)
}
