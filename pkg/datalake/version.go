...
// RollbackToVersion rolls back a Delta table to a specific version
// If forceRollback is true, the 30-day restriction is bypassed (for demo/testing purposes)
func (m *MetadataManager) RollbackToVersion(version int64, forceRollback bool) error {
	// Get all versions
	versions, err := m.GetVersions()
	if err != nil {
	...
	if err != nil {
		return fmt.Errorf("failed to get table at version %d: %w", version, err)
	}

	if !forceRollback && time.Since(table.LastModified) > 30*24*time.Hour {
		return fmt.Errorf("cannot rollback to version %d: version is older than 30 days", version)
	}
	...
	// Create a new version with the same content as the target version
	...
	logFile := filepath.Join(logDir, fmt.Sprintf("%020d.json", newVersion))
	...
	commitInfo := map[string]interface{}{
		...
		"operationParameters": map[string]string{
			"targetVersion": fmt.Sprintf("%d", version),
		},
		"isBlindAppend": false,
	...
	// Write commit info
	commitFile := filepath.Join(logDir, fmt.Sprintf("%020d.commit.json", newVersion))
	if err := os.WriteFile(commitFile, commitData, 0644); err != nil {
		// Clean up the log file if writing commit info fails
		os.Remove(logFile)
