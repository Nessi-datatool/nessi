"""
Nessi.dev client for Prefect integration.

This module provides a client for interacting with the Nessi.dev API.
"""
...
class NessiClient:
    ...
    @classmethod
    async def from_secret_block(cls, secret_block_name: str) -> "NessiClient":
        """
        Create a client from a Prefect Secret block.
        ...
        secret_value = await secret_block.get()
        
        try:
            credentials = json.loads(secret_value)
        except json.JSONDecodeError:
            raise ValueError(f"Secret block '{secret_block_name}' does not contain valid JSON")
        ...
        if not api_host:
            raise ValueError(f"Secret block '{secret_block_name}' must contain 'api_host'")
        if not api_key:
            raise ValueError(f"Secret block '{secret_block_name}' must contain 'api_key'")
    ...
    def _create_session(self) -> requests.Session:
        ...
        session.headers.update({
            ...
            "User-Agent": f"nessi-prefect/{__name__}",
        })
        ...
        try:
            ...
            if e.response.content:
                try:
                    ...
                    logger.error("Error response: %s", e.response.content)
    ...
    def wait_for_quality_results(
        ...
        while True:
            ...
            if status == 'completed':
                logger.info("Quality check completed: %s", check_id)
                return results
            elif status == 'failed':
                raise RuntimeError(f"Quality check failed: {results.get('error')}")
            elif status == 'running':
                logger.info("Quality check still running, waiting...")
    ...
    def wait_for_profile(
        ...
        while True:
            ...
            if status == 'completed':
                logger.info("Profiling completed: %s", profile_id)
                return profile
            elif status == 'failed':
                raise RuntimeError(f"Profiling failed: {profile.get('error')}")
            elif status == 'running':
                logger.info("Profiling still running, waiting...")
                time.sleep(poll_interval)
            else:
    ...
    def wait_for_validation_results(
        ...
        while True:
            ...
            if status == 'completed':
                logger.info("Validation completed: %s", validation_id)
                return results
            elif status == 'failed':
                raise RuntimeError(f"Validation failed: {results.get('error')}")
            elif status == 'running':
                logger.info("Validation still running, waiting...")
                time.sleep(poll_interval)
            else:
